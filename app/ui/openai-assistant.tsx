...
export default function OpenAIAssistant({
    ...
    return (
        <div className="flex flex-col bg-slate-200 shadow-md relative">
            <div ref={chatContainerRef} className="chat-container overflow-y-auto max-h-96 px-2">
                <OpenAIAssistantMessage message={{ id: "greeting", role: "assistant", content: greeting, createdAt: new Date() }} />
                {messages.map((m) => (
            ...
            <form onSubmit={handleSubmit} className="m-2 flex">
                <input 
                    ...
                    value={prompt}
                    placeholder="Envia un mensaje a MAAT" />
                {isLoading ? 
                    <button 
                        disabled
...
export function OpenAIAssistantMessage({message}: {message:Message}) {
    ...
    function displayRole(roleName:string) {
        switch (roleName) {
            case "user":
                return <img src="/user-logo.png" alt="User" className="role-icon" />;
            case "assistant":
                return <img src="/logo-maat.png" alt="Assistant" className="role-icon" />;
        }
    }
    return (
        <div
            className={`flex items-start rounded text-gray-700 text-center bg-white px-4 py-2 m-2 shadow-md ${
                message.role === "user" ? "flex-row-reverse bg-blue-100" : "bg-gray-100"
            }`}
        >
